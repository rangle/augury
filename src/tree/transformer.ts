...
const getDependencies: (instance: any) => Array<Dependency>
  = diagnosable({
    ...
    const normalizedParamTypes = parameterDecorators ?
      parameterTypes(instance).map((type, i) =>
        type ? type : parameterDecorators[i].find(item => item.token !== undefined).token)
      : [];
